...
Drag the Scheduler trigger on to the pipeline canvas as the first component of your data pipeline.<br />
...
On drag and drop, a page opens to configure the scheduler. At any time, you can edit the scheduler by clicking the menu dots on the scheduler and then click Scheduler in the menu.
...
| 3     | Per second scheduling      | CRON cannot run more frequently than per minute scheduling. Dataplane has a per second scheduler but it is recommended that 3 seconds is the most frequent schedule. See below on leader election and distributed locking.  |
...
| 6     | Online and offline         | You can turn on or off the scheduler. This is useful when you are busy building your pipeline and you don't want the scheduler to run. If turned on, the scheduler will go active when the pipeline is saved.               |
...
To allow for cloud native scaling and resilience, the main application which runs the scheduler can have more than one instance running as a cluster. It is recommended to run three instances as a cluster. You can see an example of this in the Kubernetes installation guide. The Kubernetes deployment yaml does this by setting the replica set to three. 

If there are three main applications running, this could also mean there are three schedulers running which can cause duplicate runs. To solve this problem, Dataplane uses two strategies to prevent duplicate runs while maintaining scaling and resilience capability. 

The first strategy is distributed locking. Each time a schedule is run, that run is locked for a period of 1 second which prevents the same scheduler running more than once at the same time. This is the reason we recommend that per second scheduling is at a minimum a conservative 3 seconds.

The second strategy is leader election. Only one leader can run the scheduler. When the main application starts up and it is the only instance running, it will elect itself as the leader. If more instances spin up, those instances contine to recognise the elected leader. The leader frequently broadcasts (or boasts) about being the leader, constantly making the other instances aware. If the leader crashes or becomes unresponisve, the other instances will randomly select a leader amongst themselves and the scheduler will be activated on that elected leader. 
